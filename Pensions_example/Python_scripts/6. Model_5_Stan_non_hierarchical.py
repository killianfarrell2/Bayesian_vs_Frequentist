...
my_data = {'N':len(X_train),'n_occ':len(np.unique(X_train)),'y':y_train,'X0':X_train[:,0],'X1':X_train[:,1] }

my_code = """
data {
      int N; // number of data points
      int<lower=0,upper=1> y[N];// data values
      int<lower=0,upper=1> X0[N];// data values
      int<lower=0,upper=1> X1[N];// data values      
}
